...
const uint8_t *const dta1[] = {band0weights, band1weights, band2weights, band3weights, band4weights, band5weights, band6weights, band7weights};
...
void analyzeAudioSamples()
{
  ...
  for (i = 0; i <= cutoffFreqBand; i++)
  {
    ...
    nBins = (uint8_t)pgm_read_byte(&bandBinCounts[i]);
    ...
    { // Get range of prior frames
      if (band[i][j] < minLvl)
        minLvl = band[i][j];
      else if (band[i][j] > maxLvl)
        maxLvl = band[i][j];
    }
    if ((maxLvl - minLvl) < 8)
      maxLvl = minLvl + 8;
...
void configure()
{
  ...
  if ((digitalRead(COLOR_BUTTON_PIN) == LOW) && (digitalRead(PATTERN_BUTTON_PIN) == HIGH))
  {
    ...
    saveConfig();
    delay(20); // debounce
    while (digitalRead(COLOR_BUTTON_PIN) == LOW)
      ; // wait for release
    return;
  }
  ...
  if ((digitalRead(PATTERN_BUTTON_PIN) == LOW) && (digitalRead(COLOR_BUTTON_PIN) == HIGH))
  {
    ...
    saveConfig();
    delay(20); // debounce
    while (digitalRead(PATTERN_BUTTON_PIN) == LOW)
      ; // wait for release
    return;
  }
...
void loop()
{
  ...
  if (transitionWaitTime == 0)
  { // only do this if not waiting to transition to new mode
