...
.macro ldiw dh, dl, abs ldi	\dl, lo8(\abs) ldi	\dh, hi8(\abs).endm
									  ...
									  .macro subiw dh,
    dl, abs subi	\dl, lo8(\abs) sbci	\dh, hi8(\abs).endm
    ...
    adc	\dh, \sh
		     ...
		     .macro addd d3,
    d2, d1, d0, s3, s2, s1, s0 add	\d0, \s0
    adc	\d1, \s1
    adc	\d2, \s2
    adc	\d3, \s3
		     .endm
    ...
    sbc	\dh, \sh
		     ...
		     .macro subd d3,
    d2, d1, d0, s3, s2, s1, s0 sub	\d0, \s0
    sbc	\d1, \s1
    sbc	\d2, \s2
    sbc	\d3, \s3
		     .endm
...
EH : zero reg.fmulsu	\s1h, \s2l
    ...
    EH
	  ...
	  .macro SQRT32;
32bit square root(526..542clk)
    clr T6L
    clr T6H
    clr T8L
    clr T8H
    ldi BL,
    1 ldi BH, 0 clr CL clr CH ldi DH, 16 90 : lsl T2L rol T2H rol T4L rol T4H rol T6L rol T6H rol T8L rol T8H lsl T2L rol T2H rol T4L rol T4H rol T6L rol T6H rol T8L rol T8H brpl 91f add T6L, BL adc T6H, BH adc T8L, CL adc T8H, CH rjmp 92f 91 : sub T6L, BL sbc T6H, BH sbc T8L, CL sbc T8H, CH 92 : lsl BL rol BH rol CL andi BL, 0b11111000 ori BL, 0b00000101 sbrc T8H, 7 subi BL, 2 dec DH brne 90b lsr CL ror BH ror BL lsr CL ror BH ror BL.endm
